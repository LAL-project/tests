...
{
	return_if(test_integer_swap()) return_if(test_integer_move())
...

	{
		std::map<
			...
			hash;
		hash.insert(
			{3, {lal::numeric::integer("42"), lal::numeric::integer("9999")}}
		);
		hash.insert(
			{4, {lal::numeric::integer("40"), lal::numeric::integer("99899")}}
		);
...
{
	return_if(test_rational_swap()) return_if(test_rational_move())
		return_if(test_rational_copy())

	{
		std::map<
			uint64_t,
			std::pair<lal::numeric::rational, lal::numeric::rational>>
			hash;
		hash.insert(
			{3,
			 {lal::numeric::rational("42/13"), lal::numeric::rational("9999")}}
		);
		hash.insert(
			{4,
			 {lal::numeric::rational("40/13"), lal::numeric::rational("99899")}}
		);
