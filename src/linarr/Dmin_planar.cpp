...
Dmin_planar_quadratic(const lal::graphs::free_tree& t) noexcept
{
...
) noexcept
{
	...
	if (Dmin_planar_quadratic.first != Dmin_planar_library.first) {
		...
		std::cerr << "        Arrangement: "
				  << Dmin_planar_library.second.direct_as_vector() << '\n';
		std::cerr << "    Quadratic: " << Dmin_planar_quadratic.first << '\n';
		std::cerr << "        Arrangement: "
				  << Dmin_planar_quadratic.second.direct_as_vector() << '\n';
		std::cerr << "    For tree:\n";
		std::cerr << T << '\n';
		return err_type::test_execution;
	}
...
err_type exe_linarr_Dmin_planar(std::ifstream& fin) noexcept
{
	...
	if (algo == "AEF" or algo == "HS") {
		...
		if (algo == "AEF") {
			...
			algo_choice =
				lal::linarr::algorithms_Dmin_planar::HochbergStallmann;
		}
		else {
			std::cerr << ERROR << '\n';
		...
		while (fin >> n) {
			if (mode == "exhaustive") {
				...
				while (not Gen.end()) {
					...
					Gen.next();
					err = tests_Dmin_planar::check_tree(
						T, algo_name, algo_choice
					);
