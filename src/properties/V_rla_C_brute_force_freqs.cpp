...
constexpr int
tau(const lal::edge& st,
	const lal::edge& uv,
	const lal::edge& wx,
	const lal::edge& yz) noexcept
{
	return static_cast<int>(st == wx or st == yz) +
		   static_cast<int>(uv == wx or uv == yz);
}

constexpr int share(const lal::edge& e1, const lal::edge& e2) noexcept
{
	return static_cast<int>(e1.first == e2.first or e1.first == e2.second) +
		   static_cast<int>(e1.second == e2.first or e1.second == e2.second);
}

constexpr int
phi(const lal::edge& st,
	const lal::edge& uv,
	const lal::edge& wx,
	const lal::edge& yz) noexcept
{
...
constexpr frequency_type
edge_pair_type(const lal::edge_pair& ep1, const lal::edge_pair& ep2) noexcept
{
